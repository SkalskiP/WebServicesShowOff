...
class HistoryViewComponent extends React.Component<Props, State> {
  public state: State = {
    tableData: [],
    totalCount: null,
    activePageIndex: 1,
  ...
  public requestData = (pageIndex: number) => {
    ...
    axios
      .get(this.url, {
        params: HistoryViewHelper.getQueryParams(this.props.activeHistoryTabName, pageIndex, this.batchSize),
      })
  ...
  protected renderTableContent = (style: React.CSSProperties) => {
    return (
      <div className="TableContent" style={style}>
        {this.state.tableData.map((data: IParkingTicket) => (
...
export const HistoryView = connect(mapStateToProps)(HistoryViewComponent);
