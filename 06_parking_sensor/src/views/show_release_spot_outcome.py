...
def show_release_spot_outcome(router):
    router.screen.clear()

    if router.soap_response and router.error_type == ErrorType.NO_ERROR:
        router.screen.addstr(1, 4, "Parking spot with id: {} is now released.".format(router.spot_id), curses.A_BOLD)
        ...
        router.screen.addstr(1, 4, "Parking spot with id: {} is already empty.".format(router.spot_id), curses.A_BOLD)
